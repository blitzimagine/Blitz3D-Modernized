...
namespace SoLoud
{
	OpenmptInstance::OpenmptInstance(Openmpt *aParent)
	{
		...
		openmpt_module_set_repeat_count((openmpt_module*)mModfile, -1);
	...
	unsigned int OpenmptInstance::getAudio(float *aBuffer, unsigned int aSamplesToRead, unsigned int aBufferSize)
	{
		...
		while (s && mPlaying)
		{
			...
			int res = openmpt_module_read_float_stereo((openmpt_module*)mModfile, (int)floor(mSamplerate), samples, aBuffer + outofs, aBuffer + outofs + aBufferSize);
			if (res == 0)
			{
				mPlaying = 0;
				return outofs;
	...
	OpenmptInstance::~OpenmptInstance()
	{
		if (mModfile)
		{
			openmpt_module_destroy((openmpt_module*)mModfile);
		}
		mModfile = 0;
	}
	...
	result Openmpt::loadFile(File *aFile)
	{
		...
		openmpt_module_destroy((openmpt_module*)mpf);
		return 0;
	}
